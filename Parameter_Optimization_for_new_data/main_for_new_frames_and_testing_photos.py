...
output_folder = "version_results_for_new_data"
...
for frame_count, lbph_param in product(frame_counts, lbph_params):
    ...
    for vid_id, name in enumerate(training_names):
        ...
        faces = []
        ids = []
        frame_paths = list(Path(f'new_frames/{vid_id}').glob('*.jpg'))
        if len(frame_paths) == 0:
            print(f"No frames found for {vid_id}. Skipping...")
            continue
    ...
    for name in training_names:
        ...
        for confidence_threshold in confidence_thresholds:
            ...
            for ID in range(1, photo_number + 1):
                img = cv2.imread(f'new_testing_photos/{ID}.jpg')
                if img is None:
                    continue
